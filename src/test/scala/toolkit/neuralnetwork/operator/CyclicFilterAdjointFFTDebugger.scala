...
/**
  ...
  */
object CyclicFilterAdjointFFTDebugger extends CogDebuggerApp(new ComputeGraph{
  val inRows = 64
  ...
  val ccFFT = realPart(fftInverse(xF*kF))

  val ccDirect = projectFrame(x,k,BorderCyclic)
  val ccDiff = ccDirect-ccFFT
  val g = ccDirect


  val g2 = VectorField.random(inShape, Shape(1))
  val xExpanded = shiftCyclic(expand(x, BorderCyclic, Shape(inRows+filterSize-1, inRows+filterSize-1)), filterSize/2, filterSize/2)
  val k2Direct = crossCorrelateFilterAdjoint(xExpanded, g2, BorderValid)
  val g2F = fft(g2)
  ...
  probe(reduceSum(fieldReduceSum(k2FFT*k)), "lhs")
