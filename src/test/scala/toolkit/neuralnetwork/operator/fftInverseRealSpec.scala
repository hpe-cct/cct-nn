...
class fftInverseRealSpec extends FunSuite with Matchers{
  ignore("FftInverseReal") {
    ...
    val vecLen = rng.nextInt(30) + 1

    println(
      s"""Running fftInverseRealSpec with parameters:
         | randomSeed: ${rng.seed}
          | rows: $rows
          | columns: $columns
          | vecLen: $vecLen
    ...
    val cg = new ComputeGraph{
      ...
      val inverse = fftInverseReal((refHalfSizedR, refHalfSizedI))

      val maxDiff = fieldReduceMax(reduceMax(abs(x - inverse)))
      probe(maxDiff)
    }
    cg.step
