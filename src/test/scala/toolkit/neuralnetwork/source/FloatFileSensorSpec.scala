...
class FloatFileSensorSpec extends FunSuite with Matchers {

  /** generate data for the test. */
  def makeRandomData(length: Int): Array[Float] = {
    val rnd = new Random
    Array.tabulate(length) (_ => rnd.nextFloat())
  }

  /** Create an automatically deleted file with the test data. */
  def makeTempInputFile(baseName: String, suffix: String, data: Array[Float]): File = {
  ...
  /** Create a vector out of the test data for the field position of the sensor ("elementIndex") as expected for
    * the given step.
    */
  def expectedVector(data:Array[Float], fieldShape: Shape, tensorElements: Int, batchSize: Int, numBatches: Int,
    ...
    Vector(tensorElements * batchSize, i => {
      ...
      data(index(imageIndex, elementIndex, vectorIndex))
    })
  }
