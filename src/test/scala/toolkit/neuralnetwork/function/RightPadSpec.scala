...
/** Tests the self-consistency of the forward/jacobian/jacobianAdjoint functions of the RightPad operator.
  *
  * @author Dick Carter
  */
class RightPadSpec extends UnitSpec with ComputeTests {
  val inputLens = Seq(31)
  val batchSizes = Seq(7)

  def fn(padSizes: Seq[Int]) = {
    a: Seq[DifferentiableField] => RightPad(a.head, padSizes)
  }

  "The RightPad operator" should "support symmetric padding" in {
  ...
  it should "support asymmetric padding" in {
