...
/** Tests the self-consistency of the forward/jacobian/jacobianAdjoint functions of the multiply operator.
  *
  * @author Matthew Pickett and Ben Chandler
  */
class MultiplySpec extends UnitSpec with ComputeTests {
  def fn(s: Seq[DifferentiableField]) = {
    require(s.length == 2)
    s.head * s(1)
  }

  "The multiplication operator" should "support 0D inputs" in {
