...
/** Tests the self-consistency of the forward/jacobian/jacobianAdjoint functions of the addition operator.
  *
  * @author Matthew Pickett and Ben Chandler
  */
class AddSpec extends UnitSpec with ComputeTests {
  def fn(s: Seq[DifferentiableField]) = {
    require(s.length == 2)
    s.head + s(1)
  }

  "The addition operator" should "support 0D inputs" in {
